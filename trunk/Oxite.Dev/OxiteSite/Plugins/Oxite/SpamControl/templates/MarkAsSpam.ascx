...
<form class="flag spam" method="post" action="<%=Url.PluginRouteUrl(((Plugin)ViewData["Plugin"]), "MarkAsSpam") %>">
    <fieldset>
        <input type="image" class="ibutton spam" src="<%=Url.PluginImagesPath(ViewData["Plugin"] as Plugin, "flag_red.png") %>" title="<%=Model.RootModel.Localize("Comment.MarkAsSpam", "This is spam!") %>" />
