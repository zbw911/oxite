...
{ %>
    <form class="editPlugin" method="POST" action="<%=Url.PluginEdit()%>">
        <input type="image" src="<%=Url.CssPath("images/page_edit.png", ViewContext)%>" alt="<%=Model.RootModel.Localize("Edit")%>" title="<%=Model.RootModel.Localize("Edit")%>" class="ibutton image edit" />
...
if (Model.PartialModel.CompilationException == null)
{
    ...
    <form class="installPlugin modifyPlugin" method="POST" action="<%=Url.PluginInstall() %>">
        <input type="image" src="<%=Url.CssPath("images/add.png", ViewContext) %>" alt="<%=Model.RootModel.Localize("Install") %>" title="<%=Model.RootModel.Localize("Install") %>" class="ibutton image add" />
        <%=Html.Hidden("virtualPath", Model.PartialModel.GetVirtualPath())%>
