...
{ %>
    ...
    {
        %>
        <p><%=Model.RootModel.Localize("Plugin.FileMissing", "This plugin's code file is missing. Try replacing this plugin's file(s) or uninstall.")%></p>
        <form method="POST" class="uninstallPlugin" action="<%=Url.PluginUninstall(plugin) %>">
            <input type="submit" value="<%=Model.RootModel.Localize("Plugin.Uninstall", "Uninstall")%>" class="submit" />
    ...
    else
    {
        if (error)
        { %>
            <p><%=Model.RootModel.Localize("Plugin.EditToFixError", "There was an error trying to load this plugin. Edit this plugin to get more info on the error and fix the problem.")%></p><%
            if (plugin != null)
            {%>
            <form method="GET" action="<%=Url.PluginEdit(plugin)%>">
                <input type="submit" value="<%=Model.RootModel.Localize("Plugin.Edit", "Edit Plugin")%>" class="submit" />
            </form><%
            }
            else
            { %>
        ...
        { %>
            <p><%=Model.RootModel.Localize("Plugin.NeedsReloadMessage", "This plugin's code file has been updated. Reload this plugin to run on the recent code.")%></p>
            <form method="POST" action="<%=Url.PluginReload(plugin) %>">
                <input type="submit" value="<%=Model.RootModel.Localize("Plugin.Reload", "Reload Plugin") %>" class="submit" />
                <%=Html.Hidden("virtualPath", Model.PartialModel.VirtualPath) %>
                <%=Html.OxiteAntiForgeryToken() %>
            </form><%
        }
...
if (plugin != null)
{
    Html.RenderPartialFromSkin(
        ...
        new OxiteViewModelPartial<Plugin>(Model.RootModel, plugin)
        );
    Html.RenderPartialFromSkin("ManagePlugin", new OxiteViewModelPartial<Plugin>(Model.RootModel, plugin));
}
else
{
    Html.RenderPartialFromSkin(
       Model.PartialModel.CompilationException == null ? "PluginInfo" : "PluginInfoError",
       new OxiteViewModelPartial<Plugin>(Model.RootModel, new Plugin(Guid.Empty, Guid.Empty, null, false) { Container = Model.PartialModel })
       );
