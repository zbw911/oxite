...
namespace Oxite.Modules.Plugins.Controllers
{
    public class PluginController : Controller
    {
        ...
        public PluginController(ISiteService siteService, IPluginService pluginService, IPluginEngine pluginEngine)
        {
        ...
        {
            return new OxiteViewModelItems<PluginContainer>
                       {
                            Items = installed != null
                                ? pluginEngine.GetPlugins(p => (bool)installed ? p.Tag is Plugin : p.Tag == null || !(p.Tag is Plugin)).FillContainer(pluginEngine)
                                : pluginEngine.GetPlugins().FillContainer(pluginEngine)
                       };
        }

        public ActionResult RefreshListNotInstalled()
        {
            pluginEngine.ReloadPlugins("~" + siteService.GetSite().PluginsPath, p => p.Tag == null || !(p.Tag is Plugin));
        ...
        public ActionResult EditNotInstalled(PluginNotInstalledAddress pluginNotInstalledAddress)
        {
            ...
            OxiteViewModelItem<PluginEditInput> model =
                new OxiteViewModelItem<PluginEditInput>
                {
                    Item = new PluginEditInput(pluginNotInstalledAddress.VirtualPath, code, false, null)
                };
        ...
        public ActionResult Uninstall(PluginAddress pluginAddress, DialogSelection dialogSelection)
        {
            ...
            pluginService.UninstallPlugin(pluginAddress, new ServiceContext(ControllerContext));
        ...
        public OxiteViewModelItem<PluginEditInput> ItemEdit(PluginAddress pluginAddress, PluginEditInput pluginEditInput)
        {
            ...
            OxiteViewModelItem<PluginEditInput> model = new OxiteViewModelItem<PluginEditInput>
            {
                Item = pluginEditInput == null
                    ? new PluginEditInput(plugin.GetFileText(), plugin.Enabled, plugin.ExtendedProperties)
                    : new PluginEditInput(null, pluginEditInput.Code ?? plugin.GetFileText(), pluginEditInput.Enabled, pluginEditInput.PropertyValues)
            };
