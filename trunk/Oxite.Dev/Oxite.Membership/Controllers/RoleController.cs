...
namespace Oxite.Modules.Membership.Controllers
{
    public class RoleController : Controller
    {
        ...
        public OxiteViewModelItems<Role> FindQuery(RoleSearchCriteria searchCriteria)
        {
            ...
            OxiteViewModelItems<Role> model = new OxiteViewModelItems<Role> { Items = foundRoles };
        ...
        public OxiteViewModelItem<Role> ItemEdit(RoleAddress roleAddress)
        {
            ...
            return new OxiteViewModelItem<Role> { Item = role };
        }

        [ActionName("ItemEdit"), AcceptVerbs(HttpVerbs.Post)]
        public object ItemSave(RoleAddress roleAddress, RoleInput roleInput, UserAuthenticated currentUser)
        {
            ServiceContext serviceContext = new ServiceContext(ControllerContext, currentUser);
            ModelResult<Role> results;

            if (roleAddress != null && !string.IsNullOrEmpty(roleAddress.RoleName))
                results = roleService.EditRole(roleAddress, roleInput, serviceContext);
            else
                results = roleService.AddRole(roleInput, serviceContext);
        ...
        public object ItemRemove(RoleAddress roleAddress, UserAuthenticated currentUser)
        {
            roleService.RemoveRole(roleAddress, new ServiceContext(ControllerContext, currentUser));
