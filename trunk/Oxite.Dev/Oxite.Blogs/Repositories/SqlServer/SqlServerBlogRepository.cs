...
namespace Oxite.Modules.Blogs.Repositories.SqlServer
{
    public class SqlServerBlogRepository : IBlogRepository
    {
        ...
        {
            return (from b in context.oxite_Blogs
        ...
        {
            oxite_Blog blogToSave = null;

            if (blog.ID != Guid.Empty)
                blogToSave = context.oxite_Blogs.FirstOrDefault(a => a.SiteID == blog.Site.ID && a.BlogID == blog.ID);

            if (blogToSave == null)
            {
                blogToSave = new oxite_Blog();
                ...
                context.oxite_Blogs.InsertOnSubmit(blogToSave);
            }
            else
        ...
        public bool Remove(Guid siteID, string blogName)
        {
            ...
            {
                oxite_Blog foundBlog = context.oxite_Blogs.FirstOrDefault(b => b.SiteID == siteID && string.Compare(b.BlogName, blogName, true) == 0);

                if (foundBlog != null)
                {
                    context.oxite_Blogs.DeleteOnSubmit(foundBlog);
        ...
        private static Blog projectBlog(oxite_Blog blog)
        {
