...
namespace Oxite.Modules.BlogML.Extensions
{
    ...
    {
        public static ImportPostInput ToImportPostInput(this BlogMLPost blogMLPost, BlogMLBlog blog, bool commentingDisabled, string slugPattern, EntityState state, UserAuthenticated creator)
        {
            ...
            if (blogMLPost.Categories != null && blogMLPost.Categories.Count > 0)
            {
                foreach (BlogMLCategoryReference bcr in blogMLPost.Categories)
                {
                    foreach (BlogMLCategory tag in blog.Categories)
                    {
                        if (tag.ID == bcr.Ref)
                        {
                            tags.Add(new Tag { ID = Guid.NewGuid(), Name = tag.Title, Created = tag.DateCreated });
            ...
            return new ImportPostInput(body, bodyShort, commentingDisabled, created, creator, modified, published, slug, state, tags, title, trackbacks);
        }

        public static ImportCommentInput ToImportCommentInput(this BlogMLComment blogMLComment, BlogMLBlog blog, UserAuthenticated user, Language language)
        {
            ...
            if (blogMLComment.UserEMail == user.Email || blogMLComment.UserEMail == blog.Authors[0].Email)
                return new ImportCommentInput(body, created, user, creatorIP, creatorUserAgent, language, modified, state);
            else
            {
                string creatorName;
                string creatorEmail;
                ...
                return new ImportCommentInput(body, created, new UserAnonymous(creatorName, creatorEmail, creatorEmailHash, creatorUrl), creatorIP, creatorUserAgent, language, modified, state);
            }
        }
    }
}
