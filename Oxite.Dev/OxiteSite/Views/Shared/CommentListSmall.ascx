<%@ Control Language="C#" Inherits="System.Web.Mvc.ViewUserControl<OxiteViewModelItemItems<Post, Comment>>" %>
<%@ Import Namespace="Oxite.Extensions" %>
<%@ Import Namespace="Oxite.Modules.Blogs.Extensions" %><%
if (((IPageOfItems<Comment>)Model.Items).TotalItemCount > 0)
{ %><ul class="comments small"><%
    int counter = 0;
    foreach (Comment postAndComment in Model.Items)
    {
    ...
    <li class="<%=className.ToString() %>">
            <div class="meta" id="<%=postAndComment.Slug %>"><%
                ...
                <span class="when"> - <%= 
                    Html.Link(
                        Html.ConvertToLocalTime(postAndComment.Created, Model).ToString("MMMM dd, yyyy - h:mm tt"), //todo: (nheskew) localize date format
