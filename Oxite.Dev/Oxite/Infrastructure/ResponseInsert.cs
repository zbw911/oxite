...
namespace Oxite.Infrastructure
{
    public class ResponseInsert
    {
        ...
        {
            Match match = closingTagRegex.Match(element);

            if (match.Success)
                return element.Replace(match.Value, value + match.Value);
        ...
        {
            Match match = openingTagRegex.Match(element);

            if (match.Success)
                return element.Replace(match.Value, match.Value + value);

            return insertBefore(element, value);
        }
