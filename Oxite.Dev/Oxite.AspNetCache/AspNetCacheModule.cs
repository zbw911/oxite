...
namespace Oxite.Modules.AspNetCache
{
    public class AspNetCacheModule : IOxiteCacheModule
    {
        ...
        public T GetItems<T, K>(string key, Func<T> getUncachedItems, Func<K, IEnumerable<ICacheItem>> getDependencies)
            where T : IEnumerable<K>
        {
            return GetItems<T, K>(key, null, getUncachedItems, getDependencies);
        }

        public T GetItems<T, K>(string key, CachePartition partition, Func<T> getUncachedItems, Func<K, IEnumerable<ICacheItem>> getDependencies)
            where T : IEnumerable<K>
        {
                ...
                if (getDependencies != null)
                    if (items.Count() > 0)
                        foreach (K item in items)
                            ...
                            foreach (ICacheItem dependency in getDependencies(item))
                        ...
                        foreach (ICacheItem dependency in getDependencies(default(K)))
                            cacheDependency(key, context, dependency);
            }
        ...
        public T GetItem<T>(string key, Func<T> getUncachedItem, Func<T, IEnumerable<ICacheItem>> getDependencies)
        {
            HttpContext context = HttpContext.Current;
        ...
        public void InvalidateItem(ICacheItem item)
        {
            HttpContext context = HttpContext.Current;

            invalidateDependency(context, item.GetCacheItemKey());

            foreach (ICacheItem cacheItem in item.GetCacheDependencyItems())
                invalidateDependency(context, cacheItem.GetCacheItemKey());
        }

        public void Invalidate<T>() where T : ICacheItem
        {
            HttpContext context = HttpContext.Current;
        ...
        private static void cacheDependency(string key, HttpContext context, ICacheItem item)
        {
